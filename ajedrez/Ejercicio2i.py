...
f4 = blan.join(negr).join(pawn.negative().up(blan)).join(pawn.negative().up(negr)).join(blan).join(negr).join(blan).join(negr)
f3 = negr.join(blan).join(negr).join(blan).join(negr).join(knight.negative().up(blan)).join(negr).join(blan)
f2 = pawn.negative().up(blan).join(pawn.negative().up(negr)).join(pawn.negative().up(blan)).join(negr).join(blan).join(pawn.negative().up(negr)).join(pawn.negative().up(blan)).join(pawn.negative().up(negr))
f1 = rock.negative().up(negr).join(knight.negative().up(blan)).join(bishop.negative().up(negr)).join(queen.negative().up(blan)).join(king.negative().up(negr)).join(bishop.negative().up(blan)).join(negr).join(rock.negative().up(blan))
